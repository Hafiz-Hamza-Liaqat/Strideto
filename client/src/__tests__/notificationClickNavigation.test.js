...
// --- 2/4/5. Bell: click navigates, marks read, navigation not gated on mark-read success ---
{
  ...
  check(
    /const markRead = async \(id\) => \{\s*await inboxApi\.markRead\(id\)\.catch\(\(\) => \{\}\);/.test(bell),
    'NotificationBell.jsx: mark-read failure is swallowed, never thrown to the UI'
  );
}
...
// --- 3/4/5/6/7. Page: click navigates, mark-read/delete are siblings (no nested controls), no navigate() in either ---
{
  ...
  check(
    /const handleActivate = \(n\) => \{\s*const safe = isSafeInternalLink\(n\.link\);\s*if \(!n\.read\) \{\s*inboxApi\.markRead\(n\._id\)\.then\(load\)\.catch\(\(\) => \{\}\);\s*\}\s*if \(safe\) navigate\(n\.link\);\s*\};/.test(page),
    'NotificationsPage.jsx: handleActivate marks read (fire-and-forget) and navigates only when the link is safe, regardless of mark-read outcome'
  );
