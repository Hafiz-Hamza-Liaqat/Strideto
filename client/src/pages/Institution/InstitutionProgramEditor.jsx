...
export default function InstitutionProgramEditor() {
  ...
  if (!canEdit || (!editing && !canCreate)) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Program authority unavailable</h1>
        <InstitutionPublishingGate authority={authority} action="program create, submit, or publish" />
        {!editing && canEdit ? null : (
          <PageState tone="warning">Approved organization verification, an approved canonical claim, and an authorized Institution role are required to create new Programs. Existing drafts may be editable once claim authority is active.</PageState>
        )}
        <Link className={secondaryButton} to={ROUTES.INSTITUTION_PROGRAMS}>Back to Programs</Link>
      </div>
    );
  }

  const select = (key, label, values) => <label className="block min-w-0 max-w-full text-sm font-medium text-gray-800 dark:text-gray-200">{label}<select className={`${fieldClass} mt-1`} value={program[key]} onChange={set(key)} title={humanize(program[key])}>{Object.values(values).map((value) => <option key={value} value={value}>{humanize(value)}</option>)}</select></label>;
