...
function ReadinessRow({ label, value, warn }) {
  return (
    <div className={`flex items-center justify-between py-2 px-3 rounded ${warn && value > 0 ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-white dark:bg-gray-800'} border border-gray-100 dark:border-gray-700 mb-1`}>
      <span className="text-sm text-gray-700 dark:text-gray-200">{label}</span>
      <span className={`text-sm font-semibold ${warn && value > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-gray-900 dark:text-white'}`}>
        {value ?? '—'}
      </span>
    </div>
  );
