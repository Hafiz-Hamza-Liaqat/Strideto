/**
 * Agent-realm secure authentication flow composition (Mission 11).
 *
 * Mirrors employerSecureAuthFlows.js for the 'agent' realm.
 * Agent supports password change (unlike Employer which has no role field).
 */
...
export function createAgentSecureAuthFlows({
  jwtProvider,
  originPolicy,
  ...
  async function resetPassword({ hashedToken, newPassword }) {
    ...
    let subject;
    try {
      subject = await agentModel.findOne(
  ...
  async function suspendAgent({ subjectId }) {
    const result = await accountSecurityMutation.suspend({ realm: REALM, subjectId });
  ...
  async function reactivateAgent({ subjectId, alsoInvalidateAccessTokens = false }) {
    return accountSecurityMutation.reactivate({ realm: REALM, subjectId, alsoInvalidateAccessTokens });
  }
...
export const agentSecureAuthFlowsHelpers = Object.freeze({ decodeExp });

export const agentSecureAuthFlows = createAgentSecureAuthFlows({
  jwtProvider: secureAuthConfig.agentJwtProvider,
