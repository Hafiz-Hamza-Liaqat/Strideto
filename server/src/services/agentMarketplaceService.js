...
export async function listPublicMarketplace({ agentType, serviceType, postType, destinationCountry, serviceCountry, language, journeyType, relatedOpportunityType, freshness, page = 1, limit = 20 } = {}) {
  ...
  const now = new Date(); const query = withFixtureExclusion({ organizationId: { $in: orgIds }, publicationStatus: PS.PUBLISHED, moderationStatus: MS.APPROVED, $and: [{ $or: [{ effectiveAt: null }, { effectiveAt: { $lte: now } }] }, { $or: [{ endsAt: null }, { endsAt: { $gt: now } }] }] });
...
export async function getPublicPost(slug) { const post = await AgentMarketplacePost.findOne(withFixtureExclusion({ slug })).lean(); if (!post || !isMarketplaceCurrentlyActive(post) || !isPubliclyLaunchVisible(post)) throw error('Marketplace post not found', 404); const approved = await OrganizationVerification.exists({ organizationId: post.organizationId, status: VERIFICATION_STATUSES.APPROVED }); if (!approved) throw error('Marketplace post not found', 404); return publicProjection(post); }
