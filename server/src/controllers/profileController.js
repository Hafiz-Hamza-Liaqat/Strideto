...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  if (req.body.onboardingGoal !== undefined) {
    user.onboardingGoal = String(req.body.onboardingGoal || '').trim().slice(0, 64);
  }
  if (req.body.careerPreferences !== undefined && req.body.careerPreferences !== null) {
    if (typeof req.body.careerPreferences === 'object' && !Array.isArray(req.body.careerPreferences)) {
      const cp = req.body.careerPreferences;
      user.careerPreferences = {
        version: Number(cp.version) || 1,
        persona: String(cp.persona || '').trim().slice(0, 64),
        lookingFor: Array.isArray(cp.lookingFor)
          ? cp.lookingFor.filter((x) => typeof x === 'string').slice(0, 20)
          : [],
        educationLevel: String(cp.educationLevel || '').trim().slice(0, 64),
        fieldsOfInterest: Array.isArray(cp.fieldsOfInterest)
          ? cp.fieldsOfInterest.filter((x) => typeof x === 'string').slice(0, 30)
          : [],
        preferredLocations: Array.isArray(cp.preferredLocations)
          ? cp.preferredLocations.filter((x) => typeof x === 'string').slice(0, 20)
          : [],
        experience: String(cp.experience || '').trim().slice(0, 64),
        careerGoal: String(cp.careerGoal || '').trim().slice(0, 64),
        notificationPrefs:
      ...
      if (user.careerPreferences.preferredLocations?.includes('Punjab')
        || user.careerPreferences.preferredLocations?.includes('Sindh')
        || user.careerPreferences.preferredLocations?.includes('Khyber Pakhtunkhwa')
        ...
        const province = user.careerPreferences.preferredLocations.find((loc) =>
          ['Punjab', 'Sindh', 'Khyber Pakhtunkhwa', 'Balochistan', 'Islamabad'].includes(loc)
        );
